...
const totalGames2023 = Object.values(stats2023Raw as Stats2023).reduce(
  (sum, m) => sum + m.success,
  0
);
...
const StorySection: React.FC<StorySectionProps> = ({
  ...
  const [showStreakVisualization, setShowStreakVisualization] =
    useState<boolean>(false);
  const [selectedRareDuo, setSelectedRareDuo] = useState<{
    player1: string;
    player2: string;
  } | null>(null);
  const [selectedDynamicDuo, setSelectedDynamicDuo] = useState<{
    player1: string;
    player2: string;
  } | null>(null);
  const [showCommunityStreakCalendar, setShowCommunityStreakCalendar] =
    useState<boolean>(false);
  const [selectedSquadSize, setSelectedSquadSize] = useState<number | null>(
    null
  );
  const [selectedSquadIndex, setSelectedSquadIndex] = useState<number | null>(
    null
  );
  ...
  const rareDuos = useMemo(() => calculateRareDuos(allPlayers), [allPlayers]);
                ...
                {perfectMonths.length > 0 && (
                  ...
                  >
                    ...
                    >
                      ...
                      <InfoTooltip text={t("stats.perfectMonthsInfo")} />
                    </div>
                  </div>
                )}
              </motion.div>
              ...
              >
                <div className="flex items-center gap-1">
                  <span
                    ...
                    style={{ backgroundColor: seasonColors.winter.bg }}
                  ></span>
                  <span
                    className="text-xs"
                    style={{ color: "var(--color-text-secondary)" }}
                ...
                <div className="flex items-center gap-1">
                  <span
                    className="w-3 h-3 rounded-full"
                    style={{ backgroundColor: seasonColors.spring.bg }}
                  ></span>
                  <span
                    className="text-xs"
                    style={{ color: "var(--color-text-secondary)" }}
                ...
                <div className="flex items-center gap-1">
                  <span
                    className="w-3 h-3 rounded-full"
                    style={{ backgroundColor: seasonColors.summer.bg }}
                  ></span>
                  <span
                    className="text-xs"
                    style={{ color: "var(--color-text-secondary)" }}
                ...
                <div className="flex items-center gap-1">
                  <span
                    className="w-3 h-3 rounded-full"
                    style={{ backgroundColor: seasonColors.autumn.bg }}
                  ></span>
                  <span
                    className="text-xs"
                    style={{ color: "var(--color-text-secondary)" }}
      ...
      content: (() => {
        ...
        return (
          <div>
            ...
            >
              ...
              >
                ...
                <div className="text-2xl font-bold">
                  {player.total2024} {t("story.games")}
                </div>
              </div>
              <div
                className="text-center px-4 py-3 rounded-xl"
              ...
              >
                ...
                <div
                  ...
                  style={{ color: "var(--color-accent-green)" }}
                >
                  {player.total2025} {t("story.games")}
                </div>
              </div>
            </motion.div>
            ...
            >
              {rankChange.direction === "up" &&
                `${t("story.rankUp")} ${rankChange.value} ${
                  rankChange.value === 1
                    ? t("story.position")
                    : t("story.positions")
                }`}
              {rankChange.direction === "down" &&
                `${t("story.rankDown")} ${rankChange.value} ${
                  rankChange.value === 1
                    ? t("story.position")
                    : t("story.positions")
                }`}
      ...
      content: (
        <div className="w-full max-w-md mx-auto">
          ...
          >
            {/* Active Months */}
            <div
              className="text-center px-4 py-3 rounded-xl flex-1"
              style={{ backgroundColor: "var(--color-bg-card)" }}
            >
          ...
          >
            ...
            >
              ...
              >
                {t("stats.clutchGames")}{" "}
                <InfoTooltip text={t("stats.clutchInfo")} />
              </div>
            </div>
            {clutchData.gamesSaved > 0 && (
              ...
              >
                ...
                >
                  {t("stats.gamesSaved")}{" "}
                  <InfoTooltip text={t("stats.gamesSavedInfo")} />
                </div>
              </div>
            )}
          </motion.div>
              ...
              {peakPerformance.startDate && peakPerformance.endDate && (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ color: "var(--color-accent-green)" }}
                  >
                    {formatDateEU(peakPerformance.startDate)} →{" "}
                    {formatDateEU(peakPerformance.endDate)}
                  </div>
                </motion.div>
              )}
          ...
          >
            <div className="flex flex-wrap gap-2 justify-center">
              {allPlayers
                .filter(
                  (p) =>
                    p.name !== player.name && (p.dates2025?.length || 0) > 0
                )
                .sort((a, b) => {
            ...
            <>
              ...
              >
                ...
                >
                  <div
                    ...
                    style={{
                      color:
                        communityStats.gamesChange >= 0
                          ? "var(--color-accent-green)"
                          : "var(--color-accent-red)",
                    }}
                  >
                    {communityStats.gamesChange >= 0 ? "+" : ""}
                    {communityStats.gamesChange}
                  </div>
                  <div
                    className="text-xs"
                    style={{ color: "var(--color-text-secondary)" }}
                ...
                >
                  ...
                  >
                    {t("community.allTime")}{" "}
                    <InfoTooltip text={t("stats.allTimeInfo")} />
                  </div>
                </div>
              </motion.div>
              ...
              >
                ...
                >
                  ...
                  {communityStreak.spansYears ? "2024-2025" : "2025"}
                </div>
                {communityStreak.startDate && communityStreak.endDate && (
                  <div
                    ...
                    style={{ color: "var(--color-accent-gold)" }}
                  >
                    {communityStreak.startDate.replace("/", ".")}.
                    {communityStreak.startYear} →{" "}
                    {communityStreak.endDate.replace("/", ".")}.
                    {communityStreak.endYear}
                  </div>
                )}
              </motion.div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1.5 justify-center">
                    {allCommunityGames
                      ...
                      .map((game, index) => {
                        const isInStreak = communityStreakDatesSet.has(
                          `${game.date}-${game.year}`
                        );
        ...
        <div>
          ...
          >
            ...
            <div
              ...
              style={{
                color:
                  communityStats.avgPlayersChange >= 0
                    ? "var(--color-accent-green)"
                    : "var(--color-accent-red)",
              }}
            >
              {communityStats.avgPlayersChange >= 0 ? "↑" : "↓"}{" "}
              {Math.abs(communityStats.avgPlayersChange)}{" "}
              {t("community.vs2024")}
            </div>
          </motion.div>
          <motion.div
          ...
          >
            ...
            <div
              ...
              style={{
                color:
                  communityStats.successRateChange >= 0
                    ? "var(--color-accent-green)"
                    : "var(--color-accent-red)",
              }}
            >
              {communityStats.successRateChange >= 0 ? "↑" : "↓"}{" "}
              {Math.abs(communityStats.successRateChange)}%{" "}
              {t("community.vs2024")}
            </div>
            <div
              ...
              style={{ color: "var(--color-text-secondary)" }}
            >
              {communityStats.gamesCancelled2025} vs{" "}
              {communityStats.gamesCancelled2024}{" "}
              {t("community.gamesCancelled")}
            </div>
          </motion.div>
        </div>
      ),
            ...
            >
              {(() => {
                const p1 = allPlayers.find(
                  (p) => p.name === selectedDynamicDuo.player1
                );
                const p2 = allPlayers.find(
                  (p) => p.name === selectedDynamicDuo.player2
                );
                ...
                return (
                  <>
                    <h3
                      ...
                      style={{ color: "var(--color-accent-green)" }}
                    >
                      {selectedDynamicDuo.player1} &{" "}
                      {selectedDynamicDuo.player2}
                    </h3>
                    <p
                    ...
                    >
                      <div className="flex items-center gap-1">
                        <span
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: duoColors.player1Only }}
                        ></span>
                        <span
                          className="text-xs"
                          style={{ color: "var(--color-text-secondary)" }}
                      ...
                      <div className="flex items-center gap-1">
                        <span
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: duoColors.player2Only }}
                        ></span>
                        <span
                          className="text-xs"
                          style={{ color: "var(--color-text-secondary)" }}
                      ...
                      <div className="flex items-center gap-1">
                        <span
                          className="w-3 h-3 rounded-full"
                          style={{ backgroundColor: duoColors.both }}
                        ></span>
                        <span
                          className="text-xs"
                          style={{ color: "var(--color-text-secondary)" }}
                      ...
                      <div className="flex items-center gap-1">
                        <span
                          className="w-3 h-3 rounded-full opacity-40"
                          style={{ backgroundColor: duoColors.neither }}
                        ></span>
                        <span
                          className="text-xs"
                          style={{ color: "var(--color-text-secondary)" }}
                        >
                          {t("stats.neitherPlayer")}
                        </span>
                      </div>
                    </motion.div>
        ...
        <div className="w-full max-w-md mx-auto">
          {!selectedRareDuo ? (
                    ...
                    {rareDuos.map((duo, index) => (
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{ color: "var(--color-text-secondary)" }}
                        >
                          {duo.player1Games} + {duo.player2Games} ={" "}
                          {duo.totalGames} {t("stats.combinedGames")}
                        </div>
                      </motion.button>
                    ))}
                  </div>
            ...
            >
              {(() => {
                const p1 = allPlayers.find(
                  (p) => p.name === selectedRareDuo.player1
                );
                const p2 = allPlayers.find(
                  (p) => p.name === selectedRareDuo.player2
                );
                const p1Dates = new Set(p1?.dates2025 || []);
                const p2Dates = new Set(p2?.dates2025 || []);
      ...
      content: (
        <div className="w-full max-w-md mx-auto">
          {selectedSquadSize === null ? (
            ...
            <>
              ...
              <div className="space-y-2">
                {repeatSquads.allResults.map((result, index) => {
                  ...
                  const isBestMatch =
                    repeatSquads.bestMatch?.squadSize === result.squadSize;
                  // Calculate total occurrences (sum of all squads)
                  const totalOccurrences = result.squads.reduce(
                    (sum, squad) => sum + squad.occurrences,
                    0
                  );
            ...
            >
              {(() => {
                const selectedResult = repeatSquads.allResults.find(
                  (r) => r.squadSize === selectedSquadSize
                );
                    ...
                    >
                      ...
                      {hasRepeats ? (
                        <div
                          ...
                          style={{ color: "var(--color-text-secondary)" }}
                        >
                          {selectedResult.squads.length}{" "}
                          {t("stats.differentSquads")} {t("stats.found")}
                        </div>
                      ) : (
                        <div
                          className="text-lg"
            ...
            >
              {(() => {
                ...
                const selectedSquad =
                  selectedResult?.squads[selectedSquadIndex];
    ...
    // Story 15: Creator's Message
    {
      content: (
        <div className="flex flex-col items-center justify-center h-full px-5 pt-20 pb-24">
          ...
          >
            {i18n.language === "bg" ? (
              <>
                ...
                в ден различен от събота! Подобрени рекорди:{" "}
          ...
          >
            {i18n.language === "bg"
              ? "Брутални мачове с драма, тръпка, смях, честна игра и без много контузии (най-важното, което не се вижда на статистиката)..."
          ...
          >
            {i18n.language === "bg"
              ? "...Правопропорционално с броя мачове вървят и караниците. Това е единствената статистика за подобряване през следващата година. Повече толериране при грешка и преглъщане на тъч/два отсъдени грешно няма да навреди на никого (надявам се)."
